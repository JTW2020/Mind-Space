...
class Eliza:
        ...
        with open(path) as file:
            ...
            for line in file:
                ...
                #The reassembly's index and rating will be supplied as well, and the index will be incremented.
                    ...
                    parts = content.split(' ')
                    decomp.reasmbs.append(Reasmb(parts, index, self.ratings[index]))
                    index += 1
    ...
    def respond(self, text):
        ...
        for key in keys:
            output, dbChanges, rindex = self._match_key(words, key, [0, 0, 0, 0])
            if output:
    ...
    def changeDatabase(self, react):
        ...
        for command in react:
            ...
            if not amount.isdigit():
                raise ValueError("A number was not supplied appropriately to react list: " + ' '.join(react))
            command = command[1:]
            ...
            if command == 'de':
                ...
                raise ValueError("The attribute " + command + " is not supported")
